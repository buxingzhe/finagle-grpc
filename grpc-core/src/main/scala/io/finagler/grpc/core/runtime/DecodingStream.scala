...
private[runtime] trait DecodingStream[T] extends Stream[T] {
  ...
  private[this] def read(s0: Buffer): Future[(Buffer, Try[Stream.Releasable[T]])] = {
    frames.read().transform {
      case Throw(rst: h2.Reset) => Future.exception(H2GrpcStatus.fromReset(rst))
      ...
      case Return(f: h2.Frame.Data) =>
        decodeFrame(s0, f) match {
          ...
          case Decoded(s1, None) =>
            if (f.isEnd) Future.value(s1 -> Throw(H2GrpcStatus.Ok()))
            else read(s1)
        }
    }
  }
...
object DecodingStream {
  ...
  def apply[T](
    ...
    protected[this] val getStatus: h2.Frame.Trailers => GrpcStatus = _ => H2GrpcStatus.Ok()
  }
  ...
  def apply[T](
    ...
    protected[this] def decoder: ByteBuffer => T = decodeF
    protected[this] val getStatus: h2.Frame.Trailers => GrpcStatus = H2GrpcStatus.fromHeaders(_)
  }
