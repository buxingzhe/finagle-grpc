...
object Codec {
    ...
    def loop(): Future[GrpcStatus] =
      stream.read().flatMap {
        case t: h2.Frame.Trailers =>
          val status = H2GrpcStatus.fromHeaders(t)
          t.release()
          Future.value(status)
        ...
        case d: h2.Frame.Data =>
          ...
          val isEnd = d.isEnd
          d.release()
          if (isEnd) Future.value(H2GrpcStatus.Unknown())
          else loop()
      }
  ...
  private[this] val decodeBufferedGrpcResponseFrame: ((ByteBuffer, GrpcStatus)) => Future[ByteBuffer] = {
    case (buf, H2GrpcStatus.Ok(_)) if buf.hasRemaining => Future(decodeGrpcFrame(buf))
