...
object Stream {
  ...
  trait Provider[-T] {
    ...
    def close(grpcStatus: GrpcStatus = H2GrpcStatus.Ok()): Future[Unit]
  }
  ...
  def fromSeq[T](seq: Seq[T]): Stream[T] = {
    ...
    q.fail(H2GrpcStatus.Ok(), discard = false)
    fromQueue(q)
  }
  ...
  def mk[T]: Stream[T] with Provider[T] = new Stream[T] with Provider[T] {
    ...
    override def close(grpcStatus: GrpcStatus = H2GrpcStatus.Ok()): Future[Unit] = {
