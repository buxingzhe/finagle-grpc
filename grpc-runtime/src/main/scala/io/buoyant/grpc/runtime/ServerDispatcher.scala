    ...
    private[this] def respondUnary[Rsp](codec: Codec[Rsp], rsp: Try[Rsp]): h2.Response = rsp match {
      case Return(msg) =>
        ...
        frames.write(h2.Frame.Data(buf, eos = false))
          .before(frames.write(H2GrpcStatus.Ok().toTrailers))
        h2.Response(h2.Status.Ok, frames)

      case Throw(e) =>
        val status = e match {
          case s: GrpcStatus => s
          case e: Throwable => H2GrpcStatus.Internal(e.getMessage)
        }
        val frames = h2.Stream()
    ...
    req.method match {
      case h2.Method.Post =>
        rpcByPath.get(req.path) match {
          ...
          case None => ServerDispatcher.fail(H2GrpcStatus.Unimplemented(req.path))
        }
      case method => ServerDispatcher.fail(H2GrpcStatus.Unknown(s"unsupported method: $method"))
    }
}
